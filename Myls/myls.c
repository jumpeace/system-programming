...
/*
    iノード：ファイルの属性を保存する場所（iノードは飛び飛びでアルファベットに従ってつけられる）
    ハードリンク：元ファイルと同じiノード指す、元ファイルを消去してもアクセス可能
    シンボリックリンク：リンク元（実体）の場所を指す（中身は空）、ディレクトリ、別パーティションでも作成可能、例：Windowsのショートカット
    ディレクトリ：所属するファイルのファイル名とiノードを記録する
    ※ハードリンクとシンボリックリンクの違い
        ：ハードリンクは対応するすべてiノードが同じだが、シンボリックリンクは違う

    参照：https://www.infraeye.com/study/linuxz28.html
*/